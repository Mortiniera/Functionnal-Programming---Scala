...
// Evaluation -----------------------------------
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    case 'def :: (Symbol(fname) :: args) ::  body :: Nil =>
      if(env == globalEnv) {
        globalEnv = env.extendRec(fname, env1 => eval('lambda :: args :: body :: Nil, env1))
      }
      
    case 'def :: (Symbol(fname) :: args) ::  body :: rest :: Nil =>
      if(env == globalEnv) {
        globalEnv = env.extendRec(fname, env1 => eval('lambda :: args :: body :: Nil, env1))
      }
      eval(rest, env.extendRec(fname, env1 => eval('lambda :: args :: body :: Nil, env1)))
      
    //if the scrutinee is equal to the value or equal to 'else, then the associated expr is returned
    case 'case :: expr_scrut :: cases =>  cases match {
      case List(value, expr) :: Nil => expr
      case List(value, expr) :: tail => 
        if(eval(expr_scrut, env) == value) expr else eval('case :: expr_scrut :: tail, env)
    }
